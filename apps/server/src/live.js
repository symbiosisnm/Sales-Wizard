...
export function attachLiveGateway(httpServer, cfg) {
    ...
    wss.on('connection', async clientWS => {
        ...
        function status(msg) {
            sendJSON(clientWS, { type: 'status', ts: now(), msg });
        }
        function error(msg) {
            sendJSON(clientWS, { type: 'error', ts: now(), msg });
        }
        ...
        clientWS.on('message', async raw => {
            ...
            if (StartSchema.safeParse(msg).success) {
                ...
                try {
                    ...
                    const config = {
                        responseModalities: responseModalities.map(m => (m === 'AUDIO' ? Modality.AUDIO : Modality.TEXT)),
